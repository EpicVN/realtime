...
export const handleImageUpload = (
  ...
  return new Promise((resolve, reject) => {
    // 1. Validate file (Giữ nguyên kiểm tra lỗi)
    if (!file) {
      reject("No file provided");
      return;
    }

    // Giới hạn 5MB (Sếp có thể sửa số này)
    const MAX_FILE_SIZE = 5 * 1024 * 1024;
    if (file.size > MAX_FILE_SIZE) {
      reject(`File quá lớn (Max ${MAX_FILE_SIZE / (1024 * 1024)}MB)`);
      return;
    }
    ...
    // Khi đọc xong -> Trả về chuỗi ảnh (Base64)
    reader.onload = () => {
      // Giả vờ load 100% để tắt thanh loading bên UI
      onProgress?.({ progress: 100 });

      if (typeof reader.result === "string") {
        resolve(reader.result); // <--- Trả về ảnh thật ở đây
      } else {
        reject("Lỗi chuyển đổi ảnh");
      }
    };
