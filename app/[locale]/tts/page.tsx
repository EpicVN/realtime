...
// GIỮ NGUYÊN VĂN BẢN MẪU (Không dịch)
...
export default function TextToSpeech() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      setError(t("error_generic")); // 🟢 Dịch thông báo lỗi chung
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-5xl mx-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <div
                        className={`p-2 rounded-full ${selectedVoice === "ngochuyen" ? "bg-pink-100 text-pink-600" : "bg-slate-100 text-slate-400"}`}
                      >
                        <FaUserAstronaut size={18} />
                      </div>
                      <div>
                        <div
                          className={`font-bold text-sm ${selectedVoice === "ngochuyen" ? "text-pink-700" : "text-slate-700"}`}
                        >
                          {t("voice_ngochuyen")} {/* Ngọc Huyền */}
                        </div>
                        <div className="text-[10px] text-slate-400">
                          {t("voice_ngochuyen_desc")} {/* Nữ • Miền Bắc */}
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <div
                        className={`p-2 rounded-full ${selectedVoice === "chieuthanh" ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-400"}`}
                      >
                        <FaUserTie size={18} />
                      </div>
                      <div>
                        <div
                          className={`font-bold text-sm ${selectedVoice === "chieuthanh" ? "text-blue-700" : "text-slate-700"}`}
                        >
                          {t("voice_chieuthanh")} {/* Chiêu Thành */}
                        </div>
                        <div className="text-[10px] text-slate-400">
                          {t("voice_chieuthanh_desc")} {/* Nam • Miền Nam */}
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-end mb-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                      {t("text_label")} {/* Nội dung kịch bản */}
                    </label>
                  ...
                  <textarea
                    ...
                    placeholder={t("placeholder")} // Nhập nội dung...
                    required
                  ></textarea>
                </div>
                ...
                {loading ? (
                  <div className="w-full py-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-center gap-3 cursor-wait shadow-inner">
                    ...
                    <span className="text-sm font-bold text-slate-500 animate-pulse">
                      {t("loading_status")} {/* Đang khởi tạo âm thanh... */}
                    </span>
                  </div>
                ) : (
                  <button
                  ...
                  >
                    ...
                    <div className="relative flex items-center justify-center gap-2 text-sm uppercase tracking-wide">
                      <FaWandMagicSparkles /> {t("btn_create")} {/* Tạo giọng đọc */}
                    </div>
                  </button>
                )}
            ...
            <div className="md:col-span-5 bg-slate-50 p-6 flex flex-col justify-center items-center border-l border-slate-100 min-h-75 md:min-h-0">
              {!audioUrl && !loading && !error && (
                <div className="text-center text-slate-400">
                  ...
                  <p className="text-xs font-medium">
                    {t("result_placeholder")} {/* Kết quả hiển thị tại đây */}
                  </p>
                </div>
              )}
              ...
              {loading && (
                <div className="text-center">
                  ...
                  <p className="text-blue-600 text-sm font-bold animate-pulse">
              ...
              {error && (
                <div className="w-full bg-red-50 border border-red-100 rounded-lg p-3 text-center animate-fade-in">
                  ...
                  <p className="text-xs text-red-600 opacity-80 wrap-break-word">
                    {error}
                  </p>
                </div>
              )}
              ...
              {audioUrl && (
                <div className="w-full animate-fade-in-up">
                  <div className="bg-white rounded-xl p-4 shadow-sm border border-green-100">
                    ...
                    >
                      <FaDownload size={14} /> {t("btn_download")} {/* Tải về */}
                    </a>
                  </div>
                </div>
              )}
        ...
        <div className="text-center mt-4">
          <p className="text-xs text-slate-400">
            {t("footer")} {/* Powered by Realtime Solutions AI */}
          </p>
        </div>
      </div>
    </div>
